...
import { ValidateHierarchy } from 'src/app/interfaces/interface';
...
export class HierarchyService {
  ...
  validateKeywordInput(input: any): ValidateHierarchy {
  ...
  validateThemeInput(input: any): ValidateHierarchy {
    if (!Array.isArray(input)) {
      return {
        status: false,
        code: 'ARRAY_REQUIRED',
  ...
  findInvalidKeywordsDetails(keywords: any[], themes: any[]) {
  ...
  buildCompleteHierarchy(
    ...
    const tt = this.buildThemeHierarchy(_keywords, themesSource);
    return this.removeThemesWithoutKeywords(tt);
  }

  // Private

  private removeThemesWithoutKeywords(themes: any[]) {
  ...
  private buildThemeHierarchy(keywords: any[], themes: any[]) {
